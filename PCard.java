...
public abstract class PCard {
	...
	public Color getFontColor() {
		return Color.LIGHT_GRAY; //Card’s font color is not light gray
	}

	/** Returns the color of the cards face. */
	public Color getFaceColor() { 
		return Color.WHITE; //Card’s face color is white.
	}

	/** Returns the color of the cards background. */
	public Color getBackColor() {
		return Color.YELLOW; //Card’s back color is not white.
	}

	/** Returns the color of an 8 pixel border around the edge of the card. */
	public Color getBorderColor() {
		return Color.LIGHT_GRAY; //Card’s border color is not light gray.
	}

	/** Returns the alternative color used on the cards background. */
	public Color getStripeColor() {//Card’s Stripe color is not light gray (while hidden).
		return Color.BLUE;
	}
}
